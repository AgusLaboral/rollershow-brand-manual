...
**Auditoría integral en un comando**: `python scripts/audit-manual.py` (contra la URL viva; con argumento `local` audita el index.html del disco). Cubre sintaxis, JSON de tokens, anclas, tells de redacción, barrido geométrico de texto pegado a hairlines en 1280/620/375, overflow, imágenes rotas, errores de consola y que el logo del header esté rotando. Correrlo antes de cerrar cualquier ronda; si reporta hallazgos, sale con código 1.
...
- v2.3: se incorporaron aprendizajes aprobados de escenas inmersivas de producto. El sistema ahora distingue translucidez Black Out, Tusor y Gasa; exige luz causal desde la abertura, caída textil prolija, perfiles full/lite equivalentes y QA visual en mobile, desktop y ultrawide. Los selectores Ancho y Alto ganaron orientación semántica y el CTA de 60px quedó documentado como perfil de escena, no como nueva base global.
- v2.4: la navegación Anterior/Siguiente de escenas inmersivas dejó de ser una mención abstracta y pasó a tener un espécimen visible y tokens productivos. Usa palabra, flecha, penumbra localizada y hairline direccional; conserva áreas táctiles explícitas y funcionamiento durante la transición.
- v2.5 (ronda de perfección, mismo ciclo): tras la marcación de aire de Agus en la secuencia de la roller, triple revisión visual por agentes + barrido geométrico texto-a-hairline. 23 defectos corregidos, entre ellos: el logo del header ahora es el SVG animado inline y rota de verdad (antes la regla 01.1 se incumplía en el propio manual), el JSON de tokens expandido se imprime indentado y legible, la firma de email recuperó sus dos líneas, los botones de escena quedaron funcionales sobre fotografía real, y una docena de ajustes de contraste, quiebre y tildes. El aparato de auditoría quedó en `scripts/audit-manual.py`.
- v2.5: por marcación de Nicolás, los valores puntuales dejaron de leerse como coordenadas obligatorias. Máxima nueva 00.4.8: las medidas documentan una intención a un tamaño de referencia y se implementan con unidades relativas, rangos fluidos, mínimos y máximos; lo que se conserva es posición relativa, proporción y jerarquía, y las buenas prácticas responsive y de accesibilidad son el piso. Quiebres de 04.5 como secuencia de colapsos, escala 04.9 acotada al espaciado interno, contenedores como anchos máximos, padding de botón como receta escalable. Sin cambios en mínimos de accesibilidad, impresos en mm, movimiento ni proporciones de 08 (valores sanos por naturaleza).
