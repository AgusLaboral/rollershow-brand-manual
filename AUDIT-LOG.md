...
| LOGO-001 | v2.5 | La regla 01.1 declara la animación de los anillos "siempre activa en digital", pero todas las instancias del manual usaban el SVG estático vía `img` (el animado trae runtime propio que no ejecuta dentro de `img`). El documento incumplía su propia regla. | El header lleva el SVG animado inline y los anillos rotan; verificado muestreando la matriz de transformación en el tiempo (chequeo incorporado a `scripts/audit-manual.py`). Los especímenes de capítulo siguen mostrando los archivos estáticos a propósito, porque documentan esas variantes. |
| DIM-002 | v2.5 | Marcación de Nicolás (2026-07-17): varios valores en px (quiebres de 04.5, pasos grandes de la escala 04.9, contenedores de 04.1) se leían como coordenadas obligatorias. Un agente que tomara el manual como ley podía escribir px fijos donde las buenas prácticas responsive piden unidades relativas y rangos fluidos, y tratar situaciones no contempladas como prohibidas. Además 04.9 ("64px entre secciones") contradecía el `--section-y` fluido de 04.1. | Máxima nueva 00.4.8 (las medidas documentan una intención; se implementan con unidades relativas y se conserva posición relativa, proporción y jerarquía), quiebres de 04.5 reencuadrados como secuencia de colapsos, escala 04.9 acotada al espaciado interno con las secciones delegadas a los tokens fluidos, contenedores declarados como anchos máximos, padding del botón declarado como receta al tamaño base con área táctil de 44px como invariante. Tokens sincronizados (`measurementRule` y reglas por bloque). Los mínimos de accesibilidad, lo impreso en mm, el movimiento y las zonas porcentuales de 08 quedaron sin cambios: son pisos, medios fijos o proporciones, y no compiten con lo responsive. |
